...
async function testRls() {
  console.log('🧪 Iniciando verificación avanzada de Row-Level Security (RLS)...');
  ...
  try {
    ...
    await AppDataSource.query(`GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO picky_rls_user;`);
    await AppDataSource.query(`GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO picky_rls_user;`);
    ...
    if (tenants.length < 2) {
      console.log('❌ Se necesitan al menos 2 tenants en la base de datos. Ejecutá primero: npm run db:seed --prefix api');
      return;
    }
    ...
    // =========================================================================
    console.log('--- CASO 1: Ejecutando consulta bajo el contexto de Picky Burgers (Rol picky_rls_user) ---');
    ...
    await runner1.query('SELECT set_config(\'app.current_tenant_id\', $1, true);', [burgerTenant.id]);
    ...
    products1.forEach(p => {
      ...
      if (!isCorrect) leaks1++;
      console.log(`   - [${isCorrect ? 'CORRECTO' : 'LEAK'}] ${p.name} (Tenant: ${p.tenantId})`);
    });
    ...
    await runner2.query('SELECT set_config(\'app.current_tenant_id\', $1, true);', [lockTenant.id]);
    ...
    if (products3.length === 0) {
      console.log('✅ EXCELENTE: RLS previno la consulta de forma predeterminada al no proveer tenant context.');
    } else {
    ...
    if (leaks1 === 0 && leaks2 === 0 && products3.length === 0) {
      console.log('🎉 VERIFICACIÓN COMPLETA: ROW-LEVEL SECURITY EN BASE DE DATOS ES 100% OPERATIVA Y SEGURA!');
      console.log('   - Aislamiento impecable entre tenants.');
      console.log('   - Prevención activa de fugas accidentales sin tenant context.');
    } else {
      console.log('⚠️ VERIFICACIÓN CON OBSERVACIONES: Se detectaron leaks en el aislamiento lógico.');
    }
    console.log('=====================================================');
