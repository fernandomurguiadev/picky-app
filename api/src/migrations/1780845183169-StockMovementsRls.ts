...
export class StockMovementsRls1780845183169 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DROP POLICY IF EXISTS sm_delete_policy ON "stock_movements"`);
    await queryRunner.query(`DROP POLICY IF EXISTS sm_update_policy ON "stock_movements"`);
    await queryRunner.query(`DROP POLICY IF EXISTS sm_insert_policy ON "stock_movements"`);
    await queryRunner.query(`DROP POLICY IF EXISTS sm_select_policy ON "stock_movements"`);
    await queryRunner.query(`ALTER TABLE "stock_movements" DISABLE ROW LEVEL SECURITY`);
  }
}
