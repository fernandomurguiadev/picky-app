...
- [x] Ejecutar en `api/`: `npm install @nestjs/websockets @nestjs/platform-socket.io socket.io`
...
- [x] Agregar `FRONTEND_URL` a `api/src/config/env.config.ts` como `z.string().url()`
...
- [x] Crear `api/src/modules/orders/orders.gateway.ts`
- [x] `@WebSocketGateway` con CORS permisivo en `handleConnection` (validar contra `FRONTEND_URL`)
- [x] Implementar `OnGatewayConnection`:
...
- [x] Implementar `OnGatewayDisconnect` (no requiere lógica adicional)
- [x] Handler `@SubscribeMessage('join-tenant')`:
...
- [x] Método `emitOrderNew(tenantId, order)` → emite `order:new` al room `tenant:{tenantId}`
- [x] Método `emitOrderStatusChanged(tenantId, payload)` → emite `order:status-changed` al room `tenant:{tenantId}`
...
- [x] Inyectar `OrdersGateway` en el constructor de `OrdersService`
- [x] Al final de `createOrder()`, después de persistir la transacción:
  `this.ordersGateway.emitOrderNew(order.tenantId, order)`
- [x] Al final de `updateOrderStatus()`, después de guardar:
...
- [x] Importar `JwtModule.registerAsync(...)` (clave pública RS256) en `OrdersModule`
- [x] Agregar `OrdersGateway` a la lista de `providers`
